...
1. **The late-phase mean-gradient component of the ER drop** (option a, now narrowed). A pre-registered post-hoc bound (`DG-0005/analyze_noise_shape.py`) showed estimator-size scaling explains the middle-phase drop on its own (share `1.03`, CI spanning 1.0) but at most `0.76` of the late-phase drop, leaving ≥20% that requires a smaller *mean* gradient or steeper-than-`1/√n` noise growth. The discriminating measurement is therefore `‖E g‖` — the **pool-mean** gradient, which is estimator-noise-free — whereas the current instrumentation records only per-batch `‖g‖`; per-step dispersion is proven unusable for this purpose here (observed `CV ≈ 0.32` against an isotropic-noise ceiling of `≈ 0.00057`). Design: at each phase boundary, compute the full-training-set (or large fixed-subset) gradient for ER in both arms and compare `‖E g‖` directly. Cost: instrumentation only, no new architecture; the existing A0/A1 arms can be re-instrumented at one screening seed. This supersedes the vaguer "variance versus convergence" framing.
